      ...
      <card v-else :title="$t('register')">
        <form @submit.prevent="register" @keydown="form.onKeydown($event)">
          ...
          <div class="mb-3 row">
            ...
            <div class="col-md-7">
              <input v-model="form.name" :class="{ 'is-invalid': form.errors.has('name') }" class="form-control" type="text" name="name" required autofocus>
  ...
  methods: {
    async register () {
      ...
      if (data.status) {
        ...
        // Log in the user.
        const { data: { token } } = await this.form.post('/api/auth/login')
